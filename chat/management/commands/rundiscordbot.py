...
async def solve(interaction: discord.Interaction, answer: str):
    try:
        ...
        if not match:
            await interaction.response.send_message(f"Puzzle {channel_id} not found. {[puz.chat_room.text_channel_id for puz in Puzzle]} (Please use this command in the puzzle-specific channel.)", ephemeral=True)
            return
